...
const Solutions = () => {
  ...
  return (
    <div className="pt-14 sm:pt-16 lg:pt-20">
      <Helmet>
        <title>Solutions | Prodynix</title>
        <meta name="description" content="Business automation, AI solutions, and custom projects. Prodynix delivers technology that solves real problems for gyms, clinics, and enterprises." />
      </Helmet>
      {/* Hero Section */}
      <section className="relative py-16 sm:py-20 lg:py-32 bg-hero-gradient bg-grid">
        <div className="absolute inset-0 overflow-hidden hidden sm:block">
          <div className="absolute top-1/4 right-1/4 w-64 sm:w-80 lg:w-96 h-64 sm:h-80 lg:h-96 bg-forge-accent/10 rounded-full blur-3xl"></div>
        </div>
        <div className="relative max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="max-w-3xl">
            ...
            <p className="text-base sm:text-lg lg:text-xl text-forge-slate-400 leading-relaxed">
              We build automation and software solutions tailored to the way your business actually works—not generic tools that force you to adapt.
            </p>
          </div>
        </div>
      </section>
      ...
      {/* Benefits Section */}
      <section className="py-12 sm:py-16 lg:py-20 bg-forge-navy">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
            {benefits.map((benefit, index) => (
              <div key={index} className="flex items-start space-x-3 sm:space-x-4">
                <div className="flex-shrink-0 w-10 h-10 sm:w-12 sm:h-12 bg-forge-accent/10 border border-forge-accent/20 rounded-lg flex items-center justify-center text-forge-accent">
                  {benefit.icon}
                </div>
                <div>
                  <h3 className="text-forge-gray-100 font-semibold mb-1 text-sm sm:text-base">{benefit.title}</h3>
                  <p className="text-forge-slate-400 text-xs sm:text-sm">{benefit.description}</p>
                </div>
              </div>
            ))}
          </div>
      ...
      {solutions.map((solution, index) => (
        <section
          key={solution.id}
          id={solution.id}
          className={`py-16 sm:py-20 lg:py-32 ${index % 2 === 1 ? 'bg-forge-navy' : 'bg-forge-darker'}`}
        >
          <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            <div className="grid lg:grid-cols-2 gap-8 sm:gap-10 lg:gap-20 items-start">
              {/* Content */}
              <div className={index % 2 === 1 ? 'lg:order-2' : ''}>
                <div className="text-forge-accent mb-3 sm:mb-4 [&>svg]:w-10 [&>svg]:h-10 sm:[&>svg]:w-12 sm:[&>svg]:h-12">
                  {solution.icon}
                </div>
                <span className="inline-block text-forge-accent font-medium text-xs sm:text-sm tracking-wider uppercase mb-2">
                  {solution.subtitle}
                </span>
                <h2 className="text-2xl sm:text-3xl lg:text-4xl font-bold text-forge-gray-50 mb-3 sm:mb-4">
                  {solution.title}
                </h2>
              ...
              {/* Features Grid */}
              <div className={`grid grid-cols-1 sm:grid-cols-2 gap-3 sm:gap-4 ${index % 2 === 1 ? 'lg:order-1' : ''}`}>
                {solution.features.map((feature, featureIndex) => (
                  <Card key={featureIndex} padding="md">
                    <h4 className="text-forge-gray-100 font-semibold mb-1 sm:mb-2 text-sm sm:text-base">{feature.title}</h4>
                    <p className="text-forge-slate-400 text-xs sm:text-sm leading-relaxed">
                      {feature.description}
                    </p>
                  </Card>
                ))}
